    ...
    <div class="row m-t-30">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.slider.update_slider_handle') }}" enctype="multipart/form-data"
                        method="post">
                        @csrf
                        <input name="slider_id" type="hidden" value="{{ $slider->id }}" readonly>
                        ...
                        <div class="form-group">
                            ...
                            <input name="discount_title" type="text" class="form-control" aria-required="true"
                                aria-invalid="false" value="{{ $slider->discount_title }}" required>
                        </div>
                        <div class="form-group">
                            <label for="old_image" class="control-label mb-1">Old Image</label>
                            <div class="product_image_container">
                                <img class="product_image" src="{{ asset('/storage/slider_image/' . $slider->image) }}">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="category_slug" class="form-select" aria-required="true" aria-invalid="false"
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->category_slug }}"
                                        @if ($category->category_slug == $slider->category_slug) selected @endif>{{ $category->category_name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
